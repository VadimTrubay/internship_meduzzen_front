export const selectMembers = (state) => state.actions.members;

export const selectAdmins = (state) => state.actions.admins;

export const selectMyInvites = (state) => state.actions.myInvites;

export const selectMyRequests = (state) => state.actions.myRequests;

export const selectCompanyInvites = (state) => state.actions.companyInvites;

export const selectCompanyRequests = (state) => state.actions.companyRequests;

export const selectLoading = (state) => state.actions.loading;

export const selectError = (state) => state.actions.error;
